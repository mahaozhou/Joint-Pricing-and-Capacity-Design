...
from simulation import simulate, cache_matches
from paths import MAIN, SIMULATION, STATE

def save(name,m,r,mode=0,warm=10.,duration=20.,seed=1910000,out_dir=SIMULATION):
    file=out_dir/f'des_{name}.json'
    if file.exists():
    ...
    np.savez_compressed(out_dir/f'des_{name}_replications.npz',values=vals)
...
def run():
    ...
    rows=json.loads((MAIN/'experiments.json').read_text())
    ...
    old=json.loads((SIMULATION/'simulation.json').read_text())
    (SIMULATION/'validation_all.json').write_text(json.dumps(old+[a,b,c,d],indent=2))
    base=json.loads((SIMULATION/'des_moderate.json').read_text())
    ...
    (STATE/'state_screening.json').write_text(json.dumps(screening,indent=2))
    state=[save('state_base',mm,r,mode=1,seed=4010000,out_dir=STATE),
           save('state_max_offline',mm,alt,mode=1,seed=4010000,out_dir=STATE),
           save('state_selected',mm,selected,mode=1,seed=4010000,out_dir=STATE)]
    (STATE/'state_validation.json').write_text(json.dumps(state,indent=2))
