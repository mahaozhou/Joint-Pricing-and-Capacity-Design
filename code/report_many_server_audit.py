...
from paths import STAFFING as OUT, TABLES
...
def build():
    ...
    original=json.loads((OUT/'staffing_transfers.json').read_text())
    referral=json.loads((OUT/'staffing_referral_transfers.json').read_text())
    transfers=[];sr=[];ur=[];pr=[]
    ...
    (TABLES/'multiserver_pricing.tex').write_text(table(
