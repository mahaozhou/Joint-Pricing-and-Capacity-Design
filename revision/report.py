...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'results/revision';FIG=ROOT/'figs/revision'
...
def write(name,text):
    ...
    (OUT/name).write_text(clean_asset(text),encoding='utf-8')
def save(fig,name):
...
def build():
    rows=read('experiments.json');transfers=read('transfers.json');staff=read('staffing_transfers.json')
    aware=read('aware_transfers.json') if (OUT/'aware_transfers.json').exists() else []
    sim=read('validation_all.json') if (OUT/'validation_all.json').exists() else read('simulation.json')
    ...
    referral_staff=read('staffing_referral_transfers.json')
    ...
    for s in sim:
        ...
        data=np.load(OUT/f'des_{s["name"]}_replications.npz')['values']
    ...
    if (OUT/'state_validation.json').exists():
        state=read('state_validation.json');lines=[]
        for s in state:
        ...
        base_values=np.load(OUT/f'des_{base["name"]}_replications.npz')['values']
        paired=[]
        for s in state[1:]:
            values=np.load(OUT/f'des_{s["name"]}_replications.npz')['values']
        ...
        (OUT/'state_paired.json').write_text(json.dumps(paired,indent=2))
